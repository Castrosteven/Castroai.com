...
const TeamMemberCard = ({ member }: { member: ITeamMember }) => {
  ...
  return (
    <div className="  h-full flex flex-col justify-between items-center  bg-gray-100 rounded-lg p-5  ">
      ...
      />
         {/* NAME */}
       <p style={{fontSize:15}} className="text-center font-bold">{name.toUpperCase()}</p>
       {/* TITLE */}
        <span style={{fontSize:15}}>{title}</span>
       </div>
        <div style={{fontSize:15}} className="font-darkGray text-darkGray ">{description}</div>
